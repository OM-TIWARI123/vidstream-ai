...
export const useVideoComment=(videoId:string,commentId:string)=>{
    ...
    const {isPending,mutate} =useMutationData(['new-comment'],(data:{comment: string})=>createCommentAndReply(user.id,data.comment,videoId,commentId),'video-comments',()=> reset())
