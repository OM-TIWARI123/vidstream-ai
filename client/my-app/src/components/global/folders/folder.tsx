...
const Folder=({id,name,optimistic,count}:Props)=>{
    ...
    return (
        <div onClick={handleFolderClick} ref={folderCardRef} className={cn(optimistic && 'opacity-60', 'flex hover:bg-neutral-800 cursor-pointer transition duration-150 items-center gap-2 justify-between min-w-[250px] py-4 px-4 rounded-lg border-[1px]')}>
            <Loader state={isPending}>
                <div className='flex flex-col gap-[1px]'>
                    ...
                    :
                     <p onClick={(e)=>e.stopPropagation()} onDoubleClick={handleNameDoubleClick} className='text-neutral-300'>{
                        latestVariables && latestVariables.status==='pending' &&
                        latestVariables.variables.id===id
                        ?  latestVariables.variables.name :name}</p>}
